...
int generer_dot_node(Node *node) {
    ...
    switch (node->type){
        ...
        case SWITCH_NODE :
            ...
            if (node->switch_node.expression) {
                int expr = generer_dot_node(node->switch_node.expression);
                fleche(id, expr);
            }

            NodeList *casesListe = node->switch_node.liste_cases;
            ...
            if (node->switch_node.default_case) {
                int id_default = generer_dot_node(node->switch_node.default_case);
                fleche(id, id_default);
            }
        ...
        case CASE_NODE :
            ...
            if (node->case_node.instruction) {
                int instrCase = generer_dot_node(node->case_node.instruction);
                fleche(id, instrCase);
            }
            if (node->case_node.saut) {
                int idSaut = generer_dot_node(node->case_node.saut);
                fleche(id, idSaut);
            }
            break;
        ...
        case DEFAULT_NODE :
            ...
            if (node->default_node.saut) {
                int idSaut = generer_dot_node(node->default_node.saut);
                fleche(id, idSaut);
            }
            break;
