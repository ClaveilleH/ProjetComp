...
// On vérifie qu'on n'a pas deux cases identiques
int chercher_case(NodeList *liste, int valeur){
    while (liste) {
        Node *case_node = liste->node;
        int val = case_node->case_node.constante->expression.valeur;
        if (val == valeur) {
            return 1;
        }
        liste = liste->suivant;
...
void afficher_node2(char *header, Node *node) {
    ...
    switch (node->type){
        ...
        case SWITCH_NODE :
            ...
            afficher_instructions2(header2, node->switch_node.liste_cases);
            afficher_node2(header2, node->switch_node.default_case);
            break;
        case CASE_NODE :
            printf("%s├── Case :\n", header);
            afficher_node2(header2, node->case_node.constante);
            afficher_node2(header2, node->case_node.instruction);
            if (node->case_node.saut) afficher_node2(header2, node->case_node.saut);

            break;
        case DEFAULT_NODE :
            printf("%s├── Default :\n", header);
            afficher_node2(header2, node->default_node.instruction);
            if (node->default_node.saut) afficher_node2(header2, node->default_node.saut);
            break;
        case FOR_NODE :
            printf("%s├── For :\n", header);
